...
\end{minted}
		...
		\todo{The difference between the two equality types was technical:
		 The first is is ad-hoc heterogeneous equality for 
		 nodes (as is different from bs), while the second is  the standard
		 homogeneous equality. 
		 For the presentation itself it does not make any difference,
		 so I am going to use the second for both.}
...
This section presents the algorithms \texttt{diff} and \texttt{diff3} and proves that they satisfy the specifications embodied respectively by \texttt{Diff} and \texttt{Diff₃}.
	...
	\todo{Nop is used only to align edit scripts in diff3, but not in the diff 
	function. I did not assign cost 0 to it because, even if it does not
	make any change, it increases the length of the edit script.
	\texttt{diff} computes an edit script with minimal length:
	for any script, a shorter, but equivalent one can be obtained
	by removing any Nop it might contain.}
...
\end{itemize}
	...
	\todo{Use different font for tools' name}
